...
let listeners: Array<(s: { exchangeRate: number; defaultCurrency: 'USD' | 'HTG' }) => void> = [];
let unsubscribe: (() => void) | null = null;

function subscribeToRate(uid: string, cb: (s: { exchangeRate: number; defaultCurrency: 'USD' | 'HTG' }) => void) {
  listeners.push(cb);
  ...
  return () => {
    ...
    if (listeners.length === 0 && unsubscribe) { unsubscribe(); unsubscribe = null; }
  };
}
...
export function useCurrency(uid?: string) {
  ...
  useEffect(() => {
    if (!uid) return;
    return subscribeToRate(uid, s => { setRate(s.exchangeRate); setDefCur(s.defaultCurrency); });
  }, [uid]);
...
export type FmtFn = ReturnType<typeof useCurrency>['fmt'];
