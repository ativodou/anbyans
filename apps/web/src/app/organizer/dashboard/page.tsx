...
export default function OrganizerDashboardPage() {
      ...
      try {
        ...
        if (eventIds.length > 0) {
          ...
          const sorted = [...tickets]
            ...
            .sort((a, b) => (b.soldAt?.seconds || b.createdAt?.seconds || 0) - (a.soldAt?.seconds || a.createdAt?.seconds || 0))
            .slice(0, 10)
            .map(t => {
  ...
  const vendorStats = vendors.map(v => ({
    ...v,
    totalOwed: vendorPurchases.filter(vp => vp.vendorId === v.id).reduce((a, vp) => a + vp.sold * vp.priceEach, 0),
  }));
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          { label: L('REVNI TOTAL', 'TOTAL REVENUE', 'REVENU TOTAL'),       value: `$${totalRevenue.toLocaleString()}`,       sub: `${totalSold} ${L('tikè vann', 'tickets sold', 'billets vendus')}` },
          ...
          { label: L('EVÈNMAN AKTIF', 'ACTIVE EVENTS', 'ÉVÉNEMENTS ACTIFS'), value: activeEvents.toString(),                  sub: `${events.length} ${L('total', 'total', 'total')}` },
          { label: L('REVANDÈ DWE', 'RESELLERS OWE', 'REVENDEURS DWE'),     value: `$${totalVendorOwed.toLocaleString()}`,    sub: `${vendors.length} ${L('revandè', 'resellers', 'revendeurs')}`, warn: totalVendorOwed > 0 },
        ].map((s, i) => (
              ...
              {events.length === 0 ? (
                ...
                return (
                  <tr key={e.id} className="border-b border-border hover:bg-white/[0.015] cursor-pointer">
                    ...
                    <td className="text-xs font-bold pl-3">${evRevenue.toLocaleString()}</td>
                  </tr>
                );
              })}
            </tbody>
              ...
              <div key={s.id} className="bg-dark-card border border-border rounded-card p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-light">
                    {s.qty || 1}× {s.section || 'GA'} · {s.source === 'vendor' ? `🏪 ${s.vendorName || L('Revandè','Reseller','Revendeur')}` : `🌐 ${L('Online','Online','En ligne')}`} · {s.buyerName || '—'}
                  </p>
                </div>
                <span className="font-heading text-lg text-green">${s.price || 0}</span>
              </div>
            ))}
          </div>
        )}
