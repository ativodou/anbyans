...
type Tab = 'dashboard' | 'events' | 'resellers' | 'revenue' | 'analytics' | 'scanner' | 'settings';
...
export default function OrganizerDashboardPage() {
  ...
  useEffect(() => {
    if (!user?.uid) return;
    getOrganizerEvents(user.uid).then(async evs => {
      setEvents(evs);
      try {
        ...
        if (eventIds.length > 0) {
          ...
          for (const chunk of chunks) {
            const q = query(collectionGroup(db, 'tickets'), where('eventId', 'in', chunk));
            const snap = await getDocs(q);
            snap.docs.forEach(d => tickets.push({ id: d.id, ...d.data() }));
          }
          setAllTickets(tickets);
        }
      } catch(e) { console.warn('analytics load err', e); }
      setLoading(false);
    ...
    <div className="min-h-screen flex bg-dark">
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {/* ═══ ANALYTICS TAB ═══ */}
          {tab === 'analytics' && (() => {
            // ── Compute metrics from allTickets ──
            ...
            const buyers = Object.values(byBuyer).map(b => ({ ...b, eventCount: b.events.size, favSection: Object.entries(b.sections).sort((a,b) => b[1]-a[1])[0]?.[0] || '—' }));
            ...
            const [aTab, setATab] = React.useState<'spenders'|'loyal'|'sections'|'events'>('spenders');

            return (
              <div>
                {validTickets.length === 0 ? (
                  <div className="text-center py-20"><div className="text-5xl mb-3">📈</div><p className="text-gray-muted text-sm">Pa gen done tikè ankò.</p></div>
                ) : (
                  <>
                    {/* KPI row */}
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
                    ...
                    <div className="flex gap-2 mb-4 overflow-x-auto">
                      {([['spenders','💸 Top Depans'],['loyal','⭐ Pli Fidèl'],['sections','🎯 Seksyon'],['events','📅 Evènman']] as const).map(([k,l]) => (
