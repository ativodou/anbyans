...
const STATUS_MAP = {
  'valid': { bg:'bg-green', border:'border-green', text:'text-green', dim:'bg-green-dim', icon:'✅', label:'VALID — ANTRE' },
  'used': { bg:'bg-orange', border:'border-orange', text:'text-orange', dim:'bg-orange-dim', icon:'⚠️', label:'DEJA ITILIZE' },
  'invalid': { bg:'bg-red', border:'border-red', text:'text-red', dim:'bg-red/10', icon:'❌', label:'ENVALID' },
  'wrong-event': { bg:'bg-red', border:'border-red', text:'text-red', dim:'bg-red/10', icon:'🚫', label:'MOVE EVÈNMAN' },
};
...
export default function ScannerPage() {
  ...
  const manualLookup = () => {
    ...
    const r: ScanResult = {
      ...
      name: 'Recherche Manuel', event: selectedEvent, section: 'GA',
    ...
    <div className="min-h-screen bg-dark flex flex-col">
      ...
      <nav className="sticky top-0 z-50 bg-dark border-b border-border px-5">
        <div className="max-w-[600px] mx-auto flex items-center h-14 gap-3">
          ...
          <span className="font-heading text-lg tracking-wide flex-1">📷 ESKANÈ TIKÈ</span>
      ...
      <div className="max-w-[600px] mx-auto w-full px-5 py-5 flex-1">
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          <div className="bg-dark-card border border-border rounded-card p-3 text-center">
            <p className="text-[9px] text-gray-muted uppercase">Eskane</p>
            <p className="font-heading text-2xl">{stats.total}</p>
          </div>
          <div className="bg-dark-card border border-border rounded-card p-3 text-center">
            <p className="text-[9px] text-gray-muted uppercase">Antre</p>
            <p className="font-heading text-2xl text-green">{stats.scanned}</p>
          </div>
          <div className="bg-dark-card border border-border rounded-card p-3 text-center">
            <p className="text-[9px] text-gray-muted uppercase">Refize</p>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            {scanning ? (
              <>
                ...
                <p className="text-sm text-orange mt-3 animate-pulse">Ap eskane...</p>
              </>
            ) : result ? (
              <div className={`text-center p-5 rounded-2xl ${st?.dim} border ${st?.border} max-w-xs`}>
                ...
                <div className="flex items-center justify-center gap-2 mt-1">
                  ...
                  <span className="text-[10px] text-gray-light">Plas {result.seat}</span>
                </div>
              ...
              <>
                ...
                <p className="text-xs text-gray-muted mt-3">Peze bouton an pou eskane QR code</p>
              </>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-3 mb-5">
          <button onClick={() => { setResult(null); simulateScan(); }}
            ...
            {result ? '📷 Eskane Ankò' : '📷 Eskane QR Code'}
          </button>
        ...
        {showManual && (
          <div className="bg-dark-card border border-border rounded-xl p-4 mb-5">
            <p className="text-[10px] uppercase tracking-widest text-orange font-bold mb-2">Antre Kòd Manyèl</p>
            <div className="flex gap-2">
              <input value={manualCode} onChange={e => setManualCode(e.target.value)}
                ...
                placeholder="ANB-XXXXXX" />
              <button onClick={manualLookup} className="px-5 py-2.5 rounded-lg bg-orange text-white text-xs font-bold hover:bg-orange/80 transition-all">Chèche</button>
            </div>
          </div>
        )}
        ...
        {/* Scan History */}
        <div>
          <h3 className="text-xs font-bold text-gray-light mb-3">Dènye Eskàn</h3>
