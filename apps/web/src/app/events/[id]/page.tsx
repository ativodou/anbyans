  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ color: '#888' }}>{t('event_id_loading')}</p>
      </div>
    </div>
  );
  ...
  if (notFound || !event) return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center', padding: 24 }}>
        ...
        <h2 style={{ color: '#fff', fontSize: 22, marginBottom: 8 }}>{t('event_id_not_found')}</h2>
        <p style={{ color: '#888', fontSize: 14, marginBottom: 24 }}>{t('event_id_invalid_link')}</p>
        <Link href="/events" style={{ color: '#06b6d4', textDecoration: 'none', fontSize: 14 }}>\u2190 {t('event_id_all_events')}</Link>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f' }}>
      <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #1e1e2e' }}>
        <Link href="/events" style={{ color: '#06b6d4', textDecoration: 'none', fontSize: 14 }}>\u2190 {t('event_id_all_events')}</Link>
        <Link href="/auth" style={{ color: '#06b6d4', textDecoration: 'none', fontSize: 13 }}>{t('auth_sign_in')}</Link>
      </div>
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '24px 16px' }}>
        ...
        {event.description && (
          <div style={{ marginBottom: 24 }}>
            <h2 style={{ color: '#fff', fontSize: 16, fontWeight: 700, marginBottom: 10 }}>{t('event_id_about')}</h2>
        ...
        {event.sections && event.sections.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            <h2 style={{ color: '#fff', fontSize: 16, fontWeight: 700, marginBottom: 12 }}>{t('event_id_tickets')}</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                return (
                  <div key={i} style={{ background: '#12121a', border: '1px solid #1e1e2e', borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', opacity: out ? 0.6 : 1 }}>
                    <div>
                      ...
                      <div style={{ color: '#666', fontSize: 12, marginTop: 2 }}>{out ? t('event_id_sold_out') : `${avail} ${t('event_id_spots_left')}`}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ color: out ? '#666' : '#06b6d4', fontSize: 18, fontWeight: 800 }}>${section.price}</div>
                      <div style={{ color: '#555', fontSize: 11 }}>{t('event_id_per_ticket')}</div>
                    </div>
                  </div>
                );
              })}
        ...
        {totalCap(event) > 0 && (
          <div style={{ marginBottom: 28 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ color: '#888', fontSize: 12 }}>{t('event_id_availability')}</span>
              <span style={{ color: isSoldOut ? '#ef4444' : '#06b6d4', fontSize: 12, fontWeight: 600 }}>
                {isSoldOut ? t('event_id_sold_out') : `${spotsLeft} ${t('event_id_left')}`}
              </span>
            </div>
        ...
        <button
          ...
          {isSoldOut ? t('event_id_ticket_sold_out') : `\ud83c\udf9f\ufe0f ${t('event_id_buy_tickets')} \u2014 $${minPrice(event)}+`}
        </button>
